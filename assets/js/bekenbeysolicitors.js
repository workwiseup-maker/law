(function ($) {
  ...
  // Date Picker
  if ($(".bekenbeysolicitors-datepicker").length) {
    $(".bekenbeysolicitors-datepicker").each(function () {
      $(this).datepicker();
    });
  }
  ...
  //accrodion
    if ($(".bekenbeysolicitors-accrodion").length) {
        var accrodionGrp = $(".bekenbeysolicitors-accrodion");
        accrodionGrp.each(function () {
            ...
            accordion.each(function () {
                $(this)
                    ...
                    .on("click", function () {
                        var clickedAccrodion = $(this).parent(); // T�klanan accrodion ��esi

                        if (clickedAccrodion.hasClass("active")) {
                            // 1. ZATEN AKT�FSE: Kapatma i�lemini yap
                            clickedAccrodion.removeClass("active");
                            clickedAccrodion.find(".accrodion-content").slideUp();

                        } else {
                            // 2. AKT�F DE��LSE: A�ma i�lemini yap (Di�erlerini kapat�p kendini a�)
                            ...
                            clickedAccrodion.addClass("active");
                            clickedAccrodion.find(".accrodion-content").slideDown();
                        }
                    });
            });
        });
  ...
  function thmOwlInit() {
    // owl slider
    let bekenbeysolicitorsowlCarousel = $(".bekenbeysolicitors-owl__carousel");
    if (bekenbeysolicitorsowlCarousel.length) {
      bekenbeysolicitorsowlCarousel.each(function () {
        let elm = $(this);
    ...
    let bekenbeysolicitorsowlCarouselNav = $(".bekenbeysolicitors-owl__carousel--custom-nav");
    if (bekenbeysolicitorsowlCarouselNav.length) {
      bekenbeysolicitorsowlCarouselNav.each(function () {
        let elm = $(this);
  ...
  function bekenbeysolicitorsSlickInit() {
    // slick slider
    let bekenbeysolicitorsslickCarousel = $(".bekenbeysolicitors-slick__carousel");
    if (bekenbeysolicitorsslickCarousel.length) {
      bekenbeysolicitorsslickCarousel.each(function () {
        let elm = $(this);
        let options = elm.data("slick-options");
        let bekenbeysolicitorsslickCarousel = elm.slick(
          "object" === typeof options ? options : JSON.parse(options)
        );
      });
    }
    let bekenbeysolicitorsslickCarouselCounter = $(
      ".bekenbeysolicitors-slick__custome-counter"
    );
    if (bekenbeysolicitorsslickCarouselCounter.length) {
      bekenbeysolicitorsslickCarouselCounter.each(function () {
        let elm = $(this);
        let options = elm.data("slick-options");
        let currentSlide;
        let slidesCount;
        let sliderCounter = document.createElement("div");
        sliderCounter.classList.add("bekenbeysolicitors-slick__counter");

        let updateSliderCounter = function (slick, currentIndex) {
          currentSlide = slick.slickCurrentSlide() + 1;
          slidesCount = slick.slideCount;
          $(sliderCounter).html(
            '<span class="bekenbeysolicitors-slick__counter__active">' +
            currentSlide +
            "</span>" +
            "" +
            "<span>" +
  ...
  $(".bekenbeysolicitors-owl__carousel").on("initialized.owl.carousel", function () {
    console.log("Carousel initialized with thumbnail dots");
  });
  /*-- Hover Tilt --*/
  let bekenbeysolicitorsTiltElm = $(".bekenbeysolicitors-tilt");
  if (bekenbeysolicitorsTiltElm.length) {
    bekenbeysolicitorsTiltElm.each(function () {
      let self = $(this);
      let options = self.data("tilt-options");
      let bekenbeysolicitorsTilt = self.tilt(
        "object" === typeof options ? options : JSON.parse(options)
      );
    });
  }
  ...
  //Strech Column
  function bekenbeysolicitors_stretch() {
    var i = $(window).width();
    $(".row .bekenbeysolicitors-stretch-element-inside-column").each(function () {
      var $this = $(this),
  ...
  bekenbeysolicitors_stretch();

  function bekenbeysolicitors_cuved_circle() {
  ...
  $(window).on("load", function () {
    ...
    bekenbeysolicitors_cuved_circle();
  });
